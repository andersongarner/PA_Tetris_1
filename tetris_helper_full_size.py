        ...
        if direction == "clockwise":
            ...
            for i in kick_translations:
                ...
                for j in range(len(self.block_positions)):
                    ...
                    if new_x < 0 or new_x > board_width or new_y >= board_height or new_y < 0 or board[new_y][
                        new_x] != blank_color:
                        issues = True
                        break
                if not issues:
        ...
        if direction == "left":
            ...
            for i in self.block_positions:
                if i[0] + self.center_position[0] + move_val < 0:
                    move_val = 0
                    continue
                if board[int(i[1] + self.center_position[1])][
                    int(i[0] + self.center_position[0] + move_val)] != blank_color:
                    move_val = 0
        ...
        while loop < board_height:
            ...
            for i in self.block_positions:
                ...
                if 0 <= x < board_width and y >= board_height or board[y][x] != blank_color:
                    ...
                    continue
        ghost = Tetrimino()
        new_color = [(self.color[0] + 3 * blank_color[0]) / 4, (self.color[1] + 3 * blank_color[1]) / 4,
                     (self.color[2] + 3 * blank_color[2]) / 4]
